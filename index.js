...
osmtogeojson = function (data, options) {
  ...
  function _convert2geoJSON(nodes, ways, rels) {
    ...
    function build_meta_information(object) { //AANGEPAST
      var res = {
        "timestamp_dt": object.timestamp,
        "version_i": object.version,
        "changeset_i": object.changeset,
        "user_s": object.user,
        "uid_i": object.uid
      };
      for (var k in res)
    ...
    for (i = 0; i < pois.length; i++) {
      ...
      for (var prop in pois[i].tags) { //Zelf toegevoegd, haalt alles uit tags en zet deze dan in de main tree. 'delete feature.tags' zorgt ervoor dat de originele tags verwijdert worden. 
        feature[prop] = pois[i].tags[prop];
        delete feature.tags;
      }
      ...
      for (var prop in coords) {
        ...
        for (var coordinate in coords[prop]) {

          if (counter === 0) {
            pushCoordinates = coords[prop][counter].toString() + '';
            counter++;
          } else {
            pushCoordinates += ', ' + coords[prop][counter].toString();
            counter = 0;
    ...
    for (var i = 0; i < rels.length; i++) {
      if ((typeof rels[i].tags != "undefined") &&
        ...
        function construct_multipolygon(tag_object, rel) {
          ...
          for (var prop in tag_object.tags) { //Zelf toegevoegd, haalt alles uit tags en zet deze dan in de main tree. 'delete feature.tags' zorgt ervoor dat de originele tags verwijdert worden. 
            feature[prop] = tag_object.tags[prop];
            delete feature.tags;
          }
